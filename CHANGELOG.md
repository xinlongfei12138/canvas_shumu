...
- [维护] 发布版本号更新，保留 ComfyUI 服务器中转、分片媒体上传和原有渠道兼容能力。

- [修复] ComfyUI 参考图片、视频和音频统一支持 1MB 分片上传并在服务器端按原始字节合并，且源图片读取和上传均绕过本地代理，避免大尺寸素材经过 Cloudflare Tunnel 时触发 524，不压缩或改变素材分辨率。
- [修复] ComfyUI 参考素材上传、工作流提交、任务轮询和结果下载绕过本地代理直连配置的 ComfyUI 地址，避免 macOS 上较大的 2K 图片被本地代理整包缓存后触发 Cloudflare 超时；其他渠道仍按原代理设置工作。
...
- [修复] Geeknow 模型列表保留服务端的端点能力信息，正确区分视频、图片、音频和文本模型，并将本地视频参考素材转换为 URL 字段，避免 multipart 文件上传被接口拒绝。
- [修复] Windows 启动器明确执行版本检查，新增独立更新批处理；更新器同时识别 Release 与主分支版本，并在启动或更新后自动修复缺失、过期依赖及重建前端和 Canvas Agent。

## v0.18.6 - 2026-09-10

- [修复] 原生 ComfyUI / MiniMax H3 任务不再沿用普通视频的 5 分钟等待上限；本地模型较慢或隧道短暂抖动时会继续使用已保存的任务 ID 轮询，避免服务器已生成而画布误报请求失败。

## v0.18.5 - 2026-09-09

- [新增] 原生 ComfyUI 视频协议支持导入 API Prompt 工作流，按画布分辨率换算 MiniMax H3 百万像素与 32 倍数尺寸，并上传参考图片、视频、音频后异步轮询本地视频结果。
- [修复] ComfyUI 工作流提交会清理模板中的历史参考素材，自动切换 H3 文生、图生、首尾帧和全能参考模式，并补齐 input 上传、大视频分片、帧数对齐与节点错误回传。

## v0.18.4 - 2026-09-08

- [修复] Windows 自动更新器不再使用非法的反斜杠正则拆分路径，修复下载新版本后源码覆盖中断并继续启动旧版本的问题。

## v0.18.3 - 2026-09-07

- [修复] Shafu `SD-720p-933` 视频模型不再发送其接口不支持的 `face_processing` 字段，其他模型行为保持不变。
...
+ [优化] 补充 Windows 新电脑的源码安装、构建和本地启动说明，并统一使用 `http://127.0.0.1:3000`，避免与 `localhost` 的浏览器本地数据混用。
+ [修复] 无痕 AI 已回写的视频结果改走带鉴权的中转下载接口保存到画布本地，避免静态视频缺少跨域响应头导致服务器已有结果但本地保存失败。
+ [修复] 无痕 AI 获取 Access Token 时补齐接口要求的 nonce 与时间戳参数，修复去字幕/水印任务提示缺少 nonce 的问题。
+ [新增] 模型渠道新增仅支持视频的 Canvas 开放视频 API 协议，支持文生视频、本地或公网图片/视频/音频参考、首尾帧、异步轮询和失败退款信息。
+ [修复] AutoDL 三个含音频输入的 MiniMax H3 工作流支持把本地 MP3、WAV、M4A/MP4 音频和 FLAC 转为完整 Data URL 提交。
+ [新增] 视频节点新增无痕 AI 去字幕/水印操作，创建相连结果节点并支持异步任务恢复、结果本地保存及远端临时文件清理。
+ [调整] 公网图片中转升级为自建公网素材中转，按需支持字字动画本地图片/视频/音频、AutoDL 本地图片及无痕视频处理，普通素材保留 24 小时。
+ [新增] 模型渠道新增火山方舟、字字动画与 AutoDL ComfyUI 协议，支持各自已核验的文本、图片、视频请求、10 个 MiniMax H3 工作流及 736p/768p 视频清晰度。
