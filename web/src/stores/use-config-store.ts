...
export type AiConfig = {
    ...
    proxyUrl: string;
    publicMediaUploadEnabled: boolean;
    publicMediaUploadUrl: string;
    publicMediaUploadToken: string;
    wuhenEnabled: boolean;
    wuhenBaseUrl: string;
    wuhenApiKey: string;
    wuhenModel: "video_removal_std" | "video_removal_pro";
};
...
const ZIZIDONGHUA_BASE_URL = "https://www.zizidonghua.com";
const AUTODL_BASE_URL = "https://autodl.art";
const CANVAS_VIDEO_BASE_URL = "https://api.canvas.12646464.xyz";
...
export const defaultConfig: AiConfig = {
    ...
    publicMediaUploadEnabled: false,
    publicMediaUploadUrl: PUBLIC_MEDIA_UPLOAD_URL,
    publicMediaUploadToken: "",
    wuhenEnabled: false,
    wuhenBaseUrl: WUHEN_BASE_URL,
    wuhenApiKey: "",
    wuhenModel: "video_removal_std",
};
...
const VIDEO_KEYWORDS = ["video", "sora", "veo", "kling", "wan", "hailuo", "seedance", "vidu", "pixverse", "runway", "luma", "hunyuan-video", "i2v", "t2v", "v2v"];
...
export function guessCapability(name: string): ModelCapability {
    ...
    if (value.includes("minimax_h3") || value.includes("minimax-h3") || value.includes("lightx2v")) return "video";
...
export function modelProviderCapabilities(config: AiConfig, value: string): ProviderModelCapabilities | undefined {
    return findChannelModel(config, value)?.model.providerCapabilities;
}
            ...
            merge: (persisted, current) => {
                ...
                return {
                    ...
                    config: {
                        ...
                        videoFaceProcessing: config.videoFaceProcessing || "false",
                        canvasImageCount: config.canvasImageCount || "3",
                        proxyEnabled: Boolean(config.proxyEnabled),
                        proxyUrl: config.proxyUrl || DEFAULT_LOCAL_PROXY_URL,
                        publicMediaUploadEnabled: Boolean(config.publicMediaUploadEnabled),
                        publicMediaUploadUrl: config.publicMediaUploadUrl || PUBLIC_MEDIA_UPLOAD_URL,
                        publicMediaUploadToken: typeof persistedConfig.publicMediaUploadToken === "string" ? persistedConfig.publicMediaUploadToken : defaultConfig.publicMediaUploadToken,
                        wuhenEnabled: Boolean(config.wuhenEnabled),
                        wuhenBaseUrl: config.wuhenBaseUrl || WUHEN_BASE_URL,
                        wuhenApiKey: config.wuhenApiKey || "",
                        wuhenModel: config.wuhenModel === "video_removal_pro" ? "video_removal_pro" : "video_removal_std",
                    },
                };
            },
        },
...
export function createModelChannel(channel?: Partial<ModelChannel>): ModelChannel {
    ...
    return {
        ...
        apiFormat,
        models: channelModelsForApiFormat(apiFormat, channel?.models),
    };
}
...
export function resolveModelRequestConfig(config: AiConfig, value: string) {
    ...
    return {
        ...
        providerCapabilities: modelProviderCapabilities(config, selectedValue),
    };
}
...
function normalizeProviderModelCapabilities(value: ProviderModelCapabilities | undefined) {
    ...
    const endpoints = uniqueStrings(value.endpoints);
    const sizes = uniqueStrings(value.sizes).filter((size) => /^\d+x\d+$/i.test(size));
    const durations = Array.from(new Set((value.durations || []).map(Number).filter((duration) => Number.isFinite(duration) && duration > 0))).sort((a, b) => a - b);
    const normalized: ProviderModelCapabilities = {
        ...(endpoints.length ? { endpoints } : {}),
        ...(sizes.length ? { sizes } : {}),
        ...(durations.length ? { durations } : {}),
        ...(typeof value.supportsImageInput === "boolean" ? { supportsImageInput: value.supportsImageInput } : {}),
        ...(typeof value.supportsSeed === "boolean" ? { supportsSeed: value.supportsSeed } : {}),
