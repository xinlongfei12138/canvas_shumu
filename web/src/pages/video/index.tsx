...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark" | "videoMode" | "videoNegativePrompt" | "videoFaceProcessing">;
...
export default function VideoPage() {
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        updateConfig("videoFaceProcessing", log.config.videoFaceProcessing || "false");
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        videoFaceProcessing: log.config?.videoFaceProcessing || "false",
    };
}
...
function buildLog({ prompt, model, config, references, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; references: ReferenceImage[]; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    const logConfig = {
        ...
        videoFaceProcessing: config.videoFaceProcessing,
    };
    return {
        id: nanoid(),
        createdAt: Date.now(),
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    return {
        ...
        videoMode: config.videoMode === "reference" ? "reference" : "frames",
        videoNegativePrompt: config.videoNegativePrompt || "",
        videoFaceProcessing: String(boolConfig(config.videoFaceProcessing, false)),
    };
}
