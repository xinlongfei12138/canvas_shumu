...
import { assertRelayConfig } from "@/services/api/public-media-upload";
import { assertWuhenConfig, cleanupWuhenRelayFiles, createWuhenRemovalTask, isWuhenTaskFailed, waitForWuhenRemovalTask, type WuhenRemovalTask } from "@/services/api/wuhen";
...
function wuhenTaskFromNode(node: CanvasNodeData): WuhenRemovalTask | null {
    const metadata = node.metadata;
    if (metadata?.videoTaskProvider !== "wuhen" || !metadata.videoTaskId || !metadata.wuhenOutputKey || !metadata.wuhenOutputUrl) return null;
    return { id: metadata.videoTaskId, inputKey: metadata.wuhenInputKey, outputKey: metadata.wuhenOutputKey, outputUrl: metadata.wuhenOutputUrl };
}
...
function InfiniteCanvasPage() {
    ...
    const completeVideoNodeTask = useCallback(
        async (nodeId: string, config: Parameters<typeof buildGenerationConfig>[0], prompt: string, images: Parameters<typeof createVideoGenerationTask>[2], signal: AbortSignal, extra: CanvasNodeData["metadata"] = {}, videos: ReferenceVideo[] = [], audios: ReferenceAudio[] = []) => {
            ...
            if (task.provider !== "plugin") {
                setNodes((prev) => prev.map((item) => (item.id === nodeId ? { ...item, metadata: { ...item.metadata, videoTaskId: task.id, videoTaskProvider: task.provider, ...(task.shafuProtocol ? { videoTaskProtocol: task.shafuProtocol } : {}), model: config.model } } : item)));
            }
    ...
    const pollVideoNodeTask = useCallback(
        async (node: CanvasNodeData, silent = false) => {
            ...
            try {
                ...
                const wuhenTask = wuhenTaskFromNode(node);
                if (!wuhenTask && !isAiConfigReady(generationConfig, generationConfig.model)) {
                    if (silent) {
                ...
                const result = wuhenTask
                    ? await waitForWuhenRemovalTask(effectiveConfig, wuhenTask, controller.signal)
                    : await waitForVideoGenerationTask(generationConfig, { id: taskId, provider: node.metadata?.videoTaskProvider === "wuhen" ? "openai" : node.metadata?.videoTaskProvider || "openai", model: generationConfig.model, ...(node.metadata?.videoTaskProtocol ? { shafuProtocol: node.metadata.videoTaskProtocol } : {}) }, { signal: controller.signal });
                const video = await storeGeneratedVideo(result);
                if (wuhenTask && !video.storageKey) throw new Error(t("wuhen.downloadFailed"));
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === node.id
                            ? applyGeneratedVideo(item, video, {
                                  prompt: item.metadata?.prompt,
                                  model: wuhenTask ? effectiveConfig.wuhenModel : generationConfig.model,
                                  ...
                                  ...(wuhenTask ? { videoTaskId: undefined, videoTaskProtocol: undefined, wuhenInputKey: undefined, wuhenOutputKey: undefined, wuhenOutputUrl: undefined } : {}),
                              })
                            : item,
                    ),
                );
                if (wuhenTask) await cleanupWuhenRelayFiles(effectiveConfig, wuhenTask);
            } catch (error) {
                if (isGenerationCanceled(error)) return;
                const errorDetails = error instanceof Error ? error.message : t("canvas.projectPage.generationFailed");
                message.error(errorDetails);
                const wuhenTask = wuhenTaskFromNode(node);
                if (wuhenTask && isWuhenTaskFailed(error)) await cleanupWuhenRelayFiles(effectiveConfig, wuhenTask);
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === node.id
                            ? {
                                  ...
                                  metadata: {
                                      ...
                                      ...(isVideoTaskFailed(error) || isWuhenTaskFailed(error) ? { videoTaskId: undefined, wuhenInputKey: undefined, wuhenOutputKey: undefined, wuhenOutputUrl: undefined } : {}),
                                  },
                              }
                            : item,
                    ),
    ...
    const handleRemoveSubtitlesWatermark = useCallback(
        async (sourceNode: CanvasNodeData) => {
            if (sourceNode.type !== CanvasNodeType.Video || !sourceNode.metadata?.content) return;
            try {
                assertWuhenConfig(effectiveConfig);
                assertRelayConfig(effectiveConfig);
            } catch (error) {
                message.error(error instanceof Error ? error.message : t("wuhen.requestFailed"));
            ...
            const resultNode = {
                ...createCanvasNode(
                    ...
                    {
                        x: sourceNode.position.x + sourceNode.width + 60 + spec.width / 2,
                        y: sourceNode.position.y + sourceNode.height / 2,
                    },
                    { status: NODE_STATUS_LOADING, model: effectiveConfig.wuhenModel },
                ),
                title: t("wuhen.resultTitle"),
            };
            ...
            setRunningNodeId(resultNode.id);
            const controller = startGenerationRequest(resultNode.id, sourceNode.id, resultNode.id);
            let task: WuhenRemovalTask | undefined;
            try {
                task = await createWuhenRemovalTask(
                    effectiveConfig,
                    {
                        name: sourceNode.title || "source-video.mp4",
                ...
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === resultNode.id
                            ? {
                                  ...item,
                                  metadata: {
                                      ...item.metadata,
                                      videoTaskId: task!.id,
                                      videoTaskProvider: "wuhen",
                                      wuhenInputKey: task!.inputKey,
                                      wuhenOutputKey: task!.outputKey,
                                      wuhenOutputUrl: task!.outputUrl,
                                  },
                              }
                            : item,
                    ),
                );
                const video = await storeGeneratedVideo(await waitForWuhenRemovalTask(effectiveConfig, task, controller.signal));
                if (!video.storageKey) throw new Error(t("wuhen.downloadFailed"));
                setNodes((prev) => prev.map((item) => (item.id === resultNode.id ? applyGeneratedVideo(item, video, { model: effectiveConfig.wuhenModel, videoTaskId: undefined, wuhenInputKey: undefined, wuhenOutputKey: undefined, wuhenOutputUrl: undefined }) : item)));
                await cleanupWuhenRelayFiles(effectiveConfig, task);
            } catch (error) {
                if (isGenerationCanceled(error)) return;
                const errorDetails = error instanceof Error ? error.message : t("wuhen.requestFailed");
                message.error(errorDetails);
                if (task && isWuhenTaskFailed(error)) await cleanupWuhenRelayFiles(effectiveConfig, task);
                setNodes((prev) =>
                    prev.map((item) =>
                        item.id === resultNode.id
                            ? {
                                  ...
                                  metadata: {
                                      ...
                                      errorDetails,
                                      ...(isWuhenTaskFailed(error) ? { videoTaskId: undefined, wuhenInputKey: undefined, wuhenOutputKey: undefined, wuhenOutputUrl: undefined } : {}),
                                  },
                              }
                            : item,
                    ),
                ...
                finishGenerationRequest(resultNode.id, controller);
                setRunningNodeId((current) => (current === resultNode.id ? null : current));
            }
        },
        ...
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                <CanvasNodeHoverToolbar
                    ...
                    onRemoveSubtitlesWatermark={(node) => void handleRemoveSubtitlesWatermark(node)}
