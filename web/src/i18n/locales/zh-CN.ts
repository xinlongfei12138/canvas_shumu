export default {
    ...
    publicMediaUpload: {
        disabled: "当前素材不是公网 URL，请先在偏好设置中开启“公网素材中转”",
        urlRequired: "请先填写公网素材中转地址",
        urlInvalid: "公网素材中转地址格式无效",
        tokenRequired: "请先填写公网素材中转令牌",
        readFailed: "读取本地参考素材失败，请重新选择素材",
        invalidMedia: "参考素材的文件类型无效",
        failed: "公网素材中转请求失败，未发送模型请求",
        missingUrl: "公网素材中转没有返回有效地址",
        invalidResponse: "公网素材中转返回的数据不完整",
    },
    wuhen: { disabled: "请先在偏好设置中开启无痕 AI", baseUrlRequired: "请先填写无痕 AI 地址", apiKeyRequired: "请先填写无痕 AI API Key", accessTokenMissing: "无痕 AI 未返回 Access Token", taskIdMissing: "无痕 AI 未返回任务 ID", requestFailed: "无痕 AI 请求失败", emptyResponse: "无痕 AI 返回为空", failed: "去字幕/水印失败", timeout: "无痕 AI 任务等待超时，可点击结果节点继续查询", downloadFailed: "处理结果未能保存到本地，请检查中转站下载配置", resultTitle: "去字幕/水印结果" },
    settingsPanels: {
        ...
        video: { title: "视频设置", quality: "清晰度", size: "尺寸", seconds: "秒数", mode: "模式", modes: { frames: "首尾帧模式", reference: "全能参考模式" }, resolution: "分辨率", ratio: "比例", duration: "时长", smart: "智能", output: "输出", generateAudio: "生成声音", watermark: "添加水印", negativePrompt: "负面提示词", negativePromptPlaceholder: "填写不希望出现在视频中的内容", faceProcessing: "参考图人脸处理", adaptive: "自适应", sizes: { landscape: "横屏", portrait: "竖屏", square: "方形", widescreen: "宽屏", tall: "长图", auto: "自动" }, ratios: { landscape: "横屏", portrait: "竖屏", square: "方形", standardLandscape: "标准横屏", standardPortrait: "标准竖屏", cinematic: "宽银幕", adaptive: "自适应" } },
    ...
    apiErrors: { requestFailed: "请求失败", requestCanceled: "请求已取消", baseUrlRequired: "请先配置 Base URL", apiKeyRequired: "请先配置 API Key", authenticationFailed: "鉴权失败，请检查 API Key、套餐权限或模型权限", rateLimited: "请求被限流或额度不足，请稍后重试", notFound: "接口地址不存在（404），请检查 Base URL 和模型选择", badGateway: "网关错误（502），接口服务暂时不可用，请稍后重试", serviceBusy: "服务繁忙（503），请稍后重试", httpFailed: "请求失败（HTTP {{status}}），请检查 Base URL 和 API Key 是否正确", htmlError: "服务返回了 HTML 错误页面（{{preview}}）", audioModelRequired: "请先配置音频模型", audioGenerationFailed: "音频生成失败", scriptNoAudio: "模型调用脚本没有返回音频", geminiAudioUnsupported: "Gemini 调用格式暂不支持音频生成，请使用 OpenAI 格式渠道", invalidImageSizeFormat: "图像尺寸格式不支持，请使用 auto、9:16 或 1024x1024", positiveImageRatio: "图像比例必须是正数，例如 9:16", imageRatioLimit: "图像宽高比不能超过 3:1，请调整尺寸", positiveImageDimensions: "图像尺寸必须是正整数，例如 1024x1024", imageDimensionStep: "图像尺寸的宽高必须是 16 的倍数，请调整尺寸", imageEdgeLimit: "图像尺寸最长边不能超过 3840px，请调整尺寸", imagePixelLimit: "图像总像素需在 655360 到 8294400 之间，请调整尺寸", unknownImageResponse: "接口返回了未知格式的数据（字段：{{fields}}），请检查模型或接口兼容性", noImageReturned: "接口没有返回图片，请检查提示词是否触发安全审核或模型是否支持该操作", geminiRejected: "Gemini 拒绝了本次请求：{{reason}}", geminiNoImage: "Gemini 接口没有返回图片", maskModelUnsupported: "蒙版编辑暂不支持该模型，请使用其他渠道", noContent: "没有返回内容", modelReadFailed: "读取模型失败", videoTimeout: "{{provider}}视频生成超时，请稍后重试", pluginVideoExpired: "插件视频任务已失效，请重新生成", scriptNoVideo: "模型调用脚本没有返回视频", noPlayableVideo: "视频接口没有返回可播放的视频", noVideoTaskId: "视频接口没有返回任务 ID", videoTaskCreateFailed: "视频任务创建失败", videoGenerationFailed: "视频生成失败", videoTaskQueryFailed: "视频任务查询失败", videoPromptRequired: "请输入视频提示词，或连接参考图片/视频/音频", referenceImageReadFailed: "参考图读取失败，请换一张图片或重新上传", invalidReferenceVideo: "参考视频必须是公网 URL、资产 ID，或本地已保存的视频", invalidReferenceAudio: "参考音频必须是公网 URL、资产 ID，或本地已保存的音频", videoModelRequired: "请先配置视频模型", geminiVideoUnsupported: "Gemini 调用格式暂不支持视频生成，请使用 OpenAI 格式渠道", noVideoTask: "接口没有返回视频任务", videoDownloadFailed: "视频下载失败", localAssetReadFailed: "读取本地资产失败", capabilityImage: "图片", capabilityText: "文本", autoDlCapabilityUnsupported: "AutoDL ComfyUI 渠道不支持{{capability}}生成", autoDlWorkflowUnknown: "AutoDL 工作流 {{workflow}} 没有已确认的请求 Schema；请提供该工作流的 API 参数后再调用", autoDlDurationRange: "当前 AutoDL 工作流只支持 {{min}}–{{max}} 秒", autoDlImageCount: "当前 AutoDL 工作流要求 {{min}}–{{max}} 张参考图", autoDlAudioCount: "当前 AutoDL 工作流要求 {{min}}–{{max}} 条参考音频", autoDlRatioUnsupported: "AutoDL 工作流不支持比例 {{ratio}}", autoDlResolutionUnsupported: "AutoDL 工作流不支持 {{resolution}}；可用值：{{supported}}", autoDlVideoReferencesUnsupported: "已适配的 AutoDL 工作流不接收参考视频", autoDlReferenceImageUrlRequired: "AutoDL 工作流文档要求参考图片使用公网 URL；当前素材不是公网 URL，未发送请求", autoDlReferenceAudioUrlRequired: "AutoDL 工作流文档要求参考音频使用公网 URL；当前素材不是公网 URL，未发送请求", providerModelListUnsupported: "火山方舟未确认可用的模型列表接口，请手动添加模型名称", ziziImageReferenceUrlRequired: "字字动画当前已确认的图生图接口要求公网图片 URL，画布本地图片不能直接发送", ziziImageReferenceLimit: "当前字字动画图片模型最多支持 {{max}} 张参考图", ziziImageReferenceSchemaUnknown: "字字动画模型 {{model}} 的参考图字段 Schema 未确认；请提供该模型文档后再调用", volcengineVideoReferencesUnsupported: "火山方舟参考素材 content 字段暂未完成协议核实；当前只支持文生视频", volcengineResolutionUnsupported: "火山方舟视频不支持 {{resolution}}，请选择 480p、720p 或 1080p", ziziReferenceVideoUrlRequired: "字字动画参考视频必须使用公网 URL", ziziReferenceAudioUrlRequired: "字字动画参考音频必须使用公网 URL", ziziAudioModelSchemaUnknown: "字字动画模型 {{model}} 的参考音频 Schema 未确认；请使用有明确音频能力的模型", ziziDurationRange: "当前字字动画视频模型只支持 {{min}}–{{max}} 秒", ziziVideoReferenceModelRequired: "当前字字动画 H3 模型不接收参考视频，请选择模型名含 -video 的视频参考模型", ziziPromoVideoUnsupported: "字字动画 H3 限时优惠系列不支持参考视频", ziziVideoModelSchemaUnknown: "字字动画模型 {{model}} 的视频请求 Schema 未确认；请提供该模型文档后再调用", ziziImageCount: "当前字字动画视频模型要求 {{min}}–{{max}} 张参考图", ziziAudioCount: "当前字字动画视频模型要求 {{min}}–{{max}} 条参考音频", ziziPromoRatioUnsupported: "字字动画 H3 限时优惠系列不支持 1:1，只支持横屏或竖屏" },
    providerErrors: {
        canvasVideoCapabilityUnsupported: "Canvas 开放视频 API 渠道只支持视频生成，不支持{{capability}}生成",
        ...
        shafuDurationUnsupported: "当前 Shafu 模型不支持 {{duration}} 秒；可用时长：{{supported}}",
        shafuSizeUnsupported: "当前 Shafu 模型不支持尺寸 {{size}}；可用尺寸：{{supported}}",
        shafuModelUnsupported: "Shafu 旧版视频协议不支持模型 {{model}}；请刷新模型列表并使用官方返回的模型",
        shafuDurationRange: "Shafu Seedance 只支持 4–15 秒整数时长",
        shafuRatioUnsupported: "Shafu Seedance 不支持比例 {{ratio}}",
        shafuFrameImageCount: "Shafu 首尾帧模式要求 1–2 张参考图",
        shafuFrameMediaUnsupported: "Shafu 首尾帧模式不接收参考视频或参考音频；请选择全能参考模式",
        shafuImageCount: "Shafu 全能参考模式最多支持 9 张参考图",
        shafuVideoCount: "Shafu 全能参考模式最多支持 3 个参考视频",
    ...
    config: {
        ...
        preferences: {
            ...
            publicMediaUpload: "公网素材中转",
            publicMediaUploadEnabled: "自动上传需要公网地址的本地素材",
            publicMediaUploadDescription: "仅当字字动画、AutoDL 或无痕 AI 的协议要求公网 URL 时按需上传；已有公网地址和支持本地直传的渠道不会重复中转。",
            publicMediaUploadUrl: "上传接口",
            publicMediaUploadToken: "上传令牌",
            publicMediaUploadPrivacy: "普通素材保留 24 小时；无痕任务素材保留 48 小时，并在结果成功保存到本地后主动删除。中转令牌保存在当前浏览器本地配置中。",
            wuhen: "无痕 AI",
            wuhenEnabled: "启用视频去字幕/水印",
            wuhenDescription: "在已有视频节点上调用无痕 AI，并创建一个相连的新视频节点等待处理结果。",
            wuhenBaseUrl: "API 地址",
            wuhenApiKey: "API Key",
            wuhenModel: "处理档位",
            wuhenStandard: "标准版",
            wuhenPro: "专业版",
            wuhenRetention: "当前按文档使用全画面自动识别；同一入口可处理字幕、文字、Logo、图片和移动水印。",
        },
        channelEditor: {
            title: "编辑渠道",
            name: "渠道名称",
