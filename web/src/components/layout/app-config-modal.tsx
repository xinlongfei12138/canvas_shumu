                    ...
                    },
                    {
                        ...
                        children: (
                            <Form layout="vertical" requiredMark={false}>
                                ...
                                <section className="mt-5 border-t border-stone-200 pt-4 dark:border-stone-800">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="min-w-0">
                                            <div className="text-sm font-semibold">{t("config.preferences.publicMediaUpload")}</div>
                                            <div className="mt-1 text-xs text-stone-500">{t("config.preferences.publicMediaUploadDescription")}</div>
                                        </div>
                                        <Switch aria-label={t("config.preferences.publicMediaUploadEnabled")} checked={config.publicMediaUploadEnabled} onChange={(checked) => updateConfig("publicMediaUploadEnabled", checked)} />
                                    </div>
                                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                                        <Form.Item label={t("config.preferences.publicMediaUploadUrl")} className="mb-0">
                                            <Input value={config.publicMediaUploadUrl} placeholder="https://update.example.com/api/assets/upload" onChange={(event) => updateConfig("publicMediaUploadUrl", event.target.value)} />
                                        </Form.Item>
                                        <Form.Item label={t("config.preferences.publicMediaUploadToken")} className="mb-0">
                                            <Input.Password value={config.publicMediaUploadToken} autoComplete="off" onChange={(event) => updateConfig("publicMediaUploadToken", event.target.value)} />
                                        </Form.Item>
                                    </div>
                                    <div className="mt-3 text-xs text-amber-700 dark:text-amber-400">{t("config.preferences.publicMediaUploadPrivacy")}</div>
                                </section>
                                <section className="mt-5 border-t border-stone-200 pt-4 dark:border-stone-800">
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="min-w-0">
                                            <div className="text-sm font-semibold">{t("config.preferences.wuhen")}</div>
                                            <div className="mt-1 text-xs text-stone-500">{t("config.preferences.wuhenDescription")}</div>
                                        </div>
                                        <Switch aria-label={t("config.preferences.wuhenEnabled")} checked={config.wuhenEnabled} onChange={(checked) => updateConfig("wuhenEnabled", checked)} />
                                    </div>
                                    <div className="mt-4 grid gap-4 md:grid-cols-2">
                                        <Form.Item label={t("config.preferences.wuhenBaseUrl")} className="mb-0">
                                            <Input value={config.wuhenBaseUrl} placeholder="https://api.wuhenai.com/v2" onChange={(event) => updateConfig("wuhenBaseUrl", event.target.value)} />
                                        </Form.Item>
                                        <Form.Item label={t("config.preferences.wuhenApiKey")} className="mb-0">
                                            <Input.Password value={config.wuhenApiKey} autoComplete="off" onChange={(event) => updateConfig("wuhenApiKey", event.target.value)} />
                                        </Form.Item>
                                        <Form.Item label={t("config.preferences.wuhenModel")} className="mb-0 md:col-span-2">
                                            <Select
                                                value={config.wuhenModel}
                                                onChange={(value) => updateConfig("wuhenModel", value)}
                                                options={[
                                                    { label: t("config.preferences.wuhenStandard"), value: "video_removal_std" },
                                                    { label: t("config.preferences.wuhenPro"), value: "video_removal_pro" },
                                                ]}
                                            />
                                        </Form.Item>
                                    </div>
                                    <div className="mt-3 text-xs text-stone-500">{t("config.preferences.wuhenRetention")}</div>
                                </section>
                            </Form>
                        ),
                    },
