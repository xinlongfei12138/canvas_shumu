...
export function ChannelEditorDrawer({ open, channel, onSave, onClose }: { open: boolean; channel: ModelChannel | null; onSave: (channel: ModelChannel) => void; onClose: () => void }) {
    ...
    const changeApiFormat = (apiFormat: ApiCallFormat) => {
        const baseUrl = apiFormat === "shafu" || !draft.baseUrl.trim() || draft.baseUrl.trim() === defaultBaseUrlForApiFormat(draft.apiFormat) ? defaultBaseUrlForApiFormat(apiFormat) : draft.baseUrl;
        patch({ apiFormat, baseUrl, models: channelModelsForApiFormat(apiFormat, draft.models) });
    };
    ...
    const save = () => {
        onSave({ ...draft, name: draft.name.trim() || t("config.channels.unnamed"), models: channelModelsForApiFormat(draft.apiFormat, draft.models) });
        onClose();
    };
                    ...
                    draft.models.map((model) => (
                        <div key={model.name} className="flex flex-wrap items-center gap-3 rounded-md px-2 py-1.5 hover:bg-stone-50 dark:hover:bg-stone-900/40">
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                                <Segmented size="small" disabled={isFixedVideoApiFormat(draft.apiFormat)} value={isFixedVideoApiFormat(draft.apiFormat) ? "video" : model.capability} options={capabilityOptions} onChange={(value) => setCapability(model.name, value as ModelCapability)} />
