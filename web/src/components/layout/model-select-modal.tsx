...
export function ModelSelectModal({ open, channel, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; onConfirm: (models: ChannelModel[]) => void; onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        setActiveTab(models.length ? "existing" : "new");
        setSearch("");
        setManual("");
    }, [open, channel]);
    ...
    const fetchModels = async () => {
        ...
        try {
            ...
            setFetched(models);
            setSelected((current) => new Set([...current, ...models.map((model) => model.name)]));
            setActiveTab("new");
    ...
    const confirm = () => {
        const ordered = new Map(existing.map((model) => [model.name, model]));
        fetched.forEach((model) => ordered.set(model.name, { ...ordered.get(model.name), ...model }));
        onConfirm(Array.from(ordered.values()).filter((model) => selected.has(model.name)));
        onClose();
    };
