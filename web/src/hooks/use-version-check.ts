...
const updateRepository = import.meta.env.VITE_UPDATE_REPOSITORY?.trim().replace(/^https?:\/\/github\.com\//, "").replace(/\.git$/, "").replace(/\/$/, "") || "xinlongfei12138/canvas_shumu";
const latestVersionUrl = updateRepository ? "https://raw.githubusercontent.com/" + updateRepository + "/main/VERSION" : "";
const latestChangelogUrl = updateRepository ? "https://raw.githubusercontent.com/" + updateRepository + "/main/CHANGELOG.md" : "";
    ...
    const checkLatestRelease = useCallback(
        async (showMessage = false) => {
            if (!latestVersionUrl || !latestChangelogUrl) {
                if (showMessage) message.info(t("version.updateUnavailable"));
                return false;
            }
            setChecking(true);
            try {
